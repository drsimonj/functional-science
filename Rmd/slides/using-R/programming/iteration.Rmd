...
Because it returns the result, we can immediately save it to a variable:
...
Using a varient of `map()`, check which columns of the `iris` data set are numeric with the function `is.numeric`.
...
Continue the pipe and regress `mpg` on `wt` for each split. Try using `lapply()` and `map()`.
